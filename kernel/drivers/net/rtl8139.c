...
static void rtl8139_poll(void *arg) {
    net_device_t *device = (net_device_t *)arg;
    ...
    while (state->rx_offset != current) {
        u8 *packet = state->rx_buffer + state->rx_offset;
        status = *(u16 *)packet;
        length = *(u16 *)(packet + 2);
        if (!(status & 1u) || length < 4u || length > 1536u) break;
        {
            net_buf_t *buffer = net_buf_clone_from_data(packet + 4, length - 4u);
            if (buffer) {
                buffer->dev = device;
...
static void rtl8139_irq_handler(registers_t *regs) {
    ...
    workqueue_schedule(rtl8139_poll, device);
}
