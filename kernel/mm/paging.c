...
static u32 alloc_page_table(void) {
    ...
    volatile uintptr_t *pt =
        (volatile uintptr_t *)phys_to_virt(pt_phys);
...
static uintptr_t *get_page_table(
    page_directory_t *pd,
    uintptr_t vaddr,
    u32 directory_flags,
    u8 create
    ...
    uintptr_t *pd_level =
        pd->page_directory +
        get_pdpt_index(vaddr) * 512u;

    pd_level[pd_idx] =
        (pt_phys & 0x000FFFFFFFFFF000ull) |
        (directory_flags &
         (PAGE_PRESENT |
          PAGE_RW |
          PAGE_USER |
...
static uintptr_t *get_page_entry(
    page_directory_t *pd,
    uintptr_t vaddr
) {
    u32 table_slot = get_table_slot(vaddr);
...
static u8 is_user_stack_address(uintptr_t addr) {
    return addr >=
               (uintptr_t)(USER_STACK_TOP -
                           USER_STACK_SIZE +
                           PAGE_SIZE) &&
           addr < (uintptr_t)USER_STACK_TOP;
}

static u8 is_user_heap_address(uintptr_t addr) {
    return addr >= (uintptr_t)USER_HEAP_START &&
           addr < (uintptr_t)USER_HEAP_END;
}

static u8 allocate_user_page(
    page_directory_t *pd,
    uintptr_t vaddr
) {
    ...
    if (!phys) {
        kprintf(
            "paging: unable to allocate user page for 0x%016llX\n",
    ...
    if (!table) {
        ...
        kprintf(
            "paging: out of page table space for 0x%016llX\n",
            (unsigned long long)vaddr
        );
    ...
    table[get_page_table_index(vaddr)] =
        (phys & 0xFFFFF000ul) |
        PAGE_PRESENT |
        PAGE_RW |
        PAGE_USER;
...
static void handle_cow_fault(
    ...
    if (!new_phys) {
        kprintf(
            "paging: COW failed, unable to allocate new page for 0x%016llX\n",
...
static void map_kernel_region(
    ...
    uintptr_t *table =
        pd->tables[table_slot];

    if (!pd->page_directory ||
        get_pml4_index(vaddr) != 0) {
        return;
    }
    ...
    if (!table) {
        ...
        if (!pt_phys) {
            panic(
                "paging: failed to allocate kernel page table"
            );
        }
        ...
        pd->page_directory[
            get_pdpt_index(vaddr) * 512u + pd_idx
        ] =
            (pt_phys & 0x000FFFFFFFFFF000ull) |
            (flags &
             (PAGE_PRESENT |
...
void paging_init(void) {
    kprintf(
        "Initializing paging system (64-bit Long Mode)...\n"
    );
    ...
    kprintf(
        "paging: kernel 64-bit identity paging active "
        "(CR3=0x%016llX)\n",
...
page_directory_t *paging_create_directory(void) {
    ...
    if (!pd_phys) {
        kprintf(
            "paging: low-memory page directory allocation failed\n"
        );

        return NULL;
    }

    volatile uintptr_t *pd_virt =
        (volatile uintptr_t *)phys_to_virt(pd_phys);
    ...
    if (!pdpt_phys || !page_directory_phys) {
        return NULL;
    }
    ...
    pd->page_directory =
        phys_to_virt(page_directory_phys);
    ...
    pd->directory[0] =
        (pdpt_phys & 0x000FFFFFFFFFF000ull) |
        PAGE_PRESENT |
        PAGE_RW |
        PAGE_USER;
...
page_directory_t *paging_create_user_directory(void) {
    ...
    if (kernel_pd && kernel_pd->directory) {
        for (u32 i = 1; i < 512; i++) {
            pd->directory[i] =
                kernel_pd->directory[i] &
                ~PAGE_USER;
        }

        for (u32 i = 0; i < 2048; i++) {
            pd->page_directory[i] =
                kernel_pd->page_directory[i] &
                ~PAGE_USER;

            if ((pd->page_directory[i] &
                 PAGE_PRESENT) &&
                !(pd->page_directory[i] & 0x80)) {
                pd->tables[i] =
                    kernel_pd->tables[i];
            }
        }
...
void paging_map(
    ...
    if (!table) {
        kprintf(
            "paging_map: Failed to allocate page table for 0x%016llX\n",
            (unsigned long long)vaddr
        );
...
/*
 * Map a kernel virtual address into the global kernel page directory.
 * This helper allows creating per-process kernel mappings (e.g. stacks)
 * while preserving the rest of kernel mappings.
 */
...
void paging_unmap_kernel(uintptr_t vaddr) {
    ...
    u32 table_slot =
        get_table_slot(vaddr);

    u32 pt_idx =
        get_page_table_index(vaddr);

    if (kernel_pd->tables[table_slot]) {
        uintptr_t *table =
            kernel_pd->tables[table_slot];
...
u8 paging_validate_user_range(
    ...
    for (
        uintptr_t page = PAGE_ALIGN_DOWN(vaddr);
        page <= PAGE_ALIGN_DOWN(end);
        page += PAGE_SIZE
    ) {
        uintptr_t *pte =
            get_page_entry(pd, page);
...
page_directory_t *paging_clone_directory(
    ...
    for (u32 table_slot = 0;
         table_slot < PAGE_TABLE_SLOTS;
         table_slot++) {

        if (!src->tables[table_slot]) {
            continue;
        }
        ...
        uintptr_t *old_pt =
            src->tables[table_slot];

        uintptr_t *new_pt =
            phys_to_virt(new_pt_phys);
        ...
        /*
         * new_pt_phys is a physical address.
         * Convert it to the kernel virtual address used
         * by the page-directory table array.
         */
        clone->tables[table_slot] =
            phys_to_virt(new_pt_phys);

        clone->page_directory[
            pdpt_idx * 512u + pd_idx
        ] =
...
paging_fault_result_t paging_handle_page_fault(
    registers_t *regs
) {
    uintptr_t fault_addr;
    ...
    u8 present =
        (regs->error_code & 0x1) ? 1 : 0;

    u8 write =
        (regs->error_code & 0x2) ? 1 : 0;

    u8 user =
        (regs->error_code & 0x4) ? 1 : 0;
    ...
    if (!present && write) {
        if (is_user_stack_address(fault_addr) ||
            is_user_heap_address(fault_addr)) {

            if (allocate_user_page(
                    paging_get_current(),
                    fault_addr)) {
